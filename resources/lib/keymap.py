...
gen_file = os.path.join(userdata, '%s.xml' % settings('keymap_editor_filename'))
...
def main():
    ...
    while not KODIMONITOR.abortRequested():
        if (settings('enable_multifile') == 'true'):
            idx = Dialog().select(tr(30000), [tr(30003), tr(30005), tr(30004), tr(30021)])
        else:
